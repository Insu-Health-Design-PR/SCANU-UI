...
function wpnTone(conf: number): 'green' | 'cyan' | 'amber' | 'red' {
  if (conf >= 0.6) return 'red';
  if (conf >= 0.35) return 'amber';
  if (conf > 0) return 'cyan';
  return 'green';
}

function wpnLabel(conf: number): string {
  if (conf >= 0.6) return `⚠ ARMADO ${(conf * 100).toFixed(0)}%`;
  if (conf >= 0.35) return `Sospechoso ${(conf * 100).toFixed(0)}%`;
  if (conf > 0) return `Observando ${(conf * 100).toFixed(0)}%`;
  return 'Limpio';
}

function stateTone(state: string): 'green' | 'cyan' | 'amber' | 'red' | 'slate' {
  if (state === 'ANOMALY_DETECTED') return 'red';
  if (state === 'ALERT') return 'red';
  if (state === 'SCANNING') return 'amber';
  if (state === 'TRIGGERED') return 'cyan';
  return 'slate';
}

function stateLabel(state: string): string {
  if (state === 'ANOMALY_DETECTED') return '⚠ ALERTA';
  return state;
}
...
export function PointCloudPanel({ compactFrame = false }: PointCloudPanelProps) {
  ...
  const weapon = useDashboardStore((state) => state.snapshot.weapon);
  ...
  const isArmed = weapon.weaponConfidence >= 0.35 || weapon.gunDetected;
  const isAlert = weapon.state === 'ANOMALY_DETECTED' || weapon.state === 'ALERT';
  ...
  return (
    <PanelCard title="Point Cloud" icon={isAlert ? <Crosshair className="h-4 w-4 text-red-400" /> : <Radar className="h-4 w-4" />}>
      <div className="mb-3 flex flex-wrap gap-2">
        ...
        <StatusChip label={stateLabel(weapon.state)} tone={stateTone(weapon.state)} />
        <StatusChip label={wpnLabel(weapon.weaponConfidence)} tone={wpnTone(weapon.weaponConfidence)} />
        {weapon.gunDetected && <StatusChip label="🔫 Cam detect" tone="red" />}
      </div>
      <div
        className={cn(
          'relative overflow-hidden rounded-[1.2rem] border transition-all duration-500',
          isAlert
            ? 'border-red-400/40 shadow-[0_0_30px_rgba(239,68,68,0.25)]'
            : isArmed
              ? 'border-amber-400/20 shadow-[0_0_20px_rgba(245,158,11,0.15)]'
              : 'border-cyan-400/10'
        )}
      >
        <div
          className={cn(
            'relative bg-[radial-gradient(circle_at_50%_92%,rgba(45,212,191,0.18),transparent_32%),linear-gradient(180deg,#08111f,#05070e)]',
            frameClassName
          )}
        >
          ...
          <div className={cn('absolute bottom-[7%] left-1/2 h-4 w-4 -translate-x-1/2 rounded-full border transition-all duration-300', isAlert ? 'border-red-300/60 bg-red-300/30 shadow-[0_0_28px_rgba(239,68,68,0.5)]' : 'border-cyan-300/40 bg-cyan-300/20 shadow-[0_0_22px_rgba(34,211,238,0.35)]')} />
          ...
          {isAlert && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <span className="animate-pulse text-5xl font-bold text-red-400/30 select-none">⚠</span>
            </div>
          )}
          ...
          {hasPoints ? (
            renderPoints.map((point) => (
              <span
                key={point.id}
                className={cn('absolute -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/40', isAlert && 'border-red-400/60')}
                ...
                style={{
                  ...
                  backgroundColor: isAlert ? '#f87171' : point.color,
                  boxShadow: isAlert ? '0 0 18px #f87171' : `0 0 18px ${point.color}`,
                }}
              />
            ))
          ) : (
      ...
      <div className="mt-3 grid gap-2 text-xs text-slate-400 md:grid-cols-6">
        ...
        <span className={weapon.weaponConfidence >= 0.35 ? 'text-amber-400' : ''}>WPN {(weapon.weaponConfidence * 100).toFixed(0)}%</span>
        <span className={weapon.microDopplerBw > 0.05 ? 'text-cyan-400' : ''}>μDop {weapon.microDopplerBw.toFixed(2)}</span>
      </div>
